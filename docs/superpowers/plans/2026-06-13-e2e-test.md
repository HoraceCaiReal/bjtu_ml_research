# E2E Test Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `scripts/e2e_test.py` — a single-file, self-contained end-to-end test that verifies the crack image recognition system works across all 7 stages (env → data → features → traditional models → CNN → Gradio module → Gradio pipeline combos).

**Architecture:** One Python script with a `StageRunner` helper class that tracks pass/fail per stage and prints a formatted report. Stage 6 calls `gradio_app.run_pipeline()` directly with a mock `progress` object, bypassing the Gradio UI.

**Tech Stack:** Python 3.10, conda env `bjtu_ml`, torch, scikit-learn, joblib, opencv, gradio (import only for module check + pipeline call).

**Spec:** `docs/superpowers/specs/2026-06-13-e2e-test-design.md`

---

### Task 1: Create script skeleton with StageRunner and Stage 0

**Files:**
- Create: `scripts/e2e_test.py`

- [ ] **Step 1: Write the script skeleton with StageRunner helper and Stage 0**
...
class StageRunner:
    ...
    def run(self, name: str, fn):
        ...
        try:
            ...
            self.results.append({
                "name": name, "passed": True, "detail": detail or "", "elapsed": elapsed,
            })
            ...
            self.results.append({
                "name": name, "passed": False, "detail": str(e), "elapsed": elapsed,
            })
...
def stage0_env() -> str:
    ...
    missing = []
    for mod in ["torch", "sklearn", "cv2", "xgboost", "lightgbm",
                "gradio", "skimage", "joblib", "dotenv"]:
        try:
            __import__(mod)
        except ImportError:
            missing.append(mod)
...
- [ ] **Step 2: Run the script to verify Stage 0 works**

Run: `conda activate bjtu_ml && python scripts/e2e_test.py`
Expected: Stage 0 prints `✅ PASS  torch X.Y.Z, CUDA: True/False` and exits 0.

- [ ] **Step 3: Commit**

```bash
git add scripts/e2e_test.py
git commit -m "feat: e2e test skeleton with StageRunner and Stage 0 env check"
```

---

### Task 2: Stage 1 — Data path verification

**Files:**
- Modify: `scripts/e2e_test.py`

- [ ] **Step 1: Add Stage 1 function before `main()`**
...
- [ ] **Step 2: Register Stage 1 in `main()`**

在 `runner.run("Stage 0: 环境检查", stage0_env)` 之后添加：
...
Run: `conda activate bjtu_ml && python scripts/e2e_test.py`
Expected: Stage 1 prints `✅ PASS  N images, 抽样读取 OK (H, W)`.

- [ ] **Step 4: Commit**

```bash
git add scripts/e2e_test.py
git commit -m "feat: e2e test Stage 1 data path verification"
```

---

### Task 3: Stage 2 — Feature extraction verification

**Files:**
- Modify: `scripts/e2e_test.py`

- [ ] **Step 1: Add Stage 2 function**
...
def stage2_features() -> str:
    ...
    # 取一张 Positive 图片
    IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".bmp"}
    sample = next(p for p in (data_root / "Positive").iterdir() if p.suffix.lower() in IMAGE_EXTS)
    buf = np.fromfile(str(sample), dtype=np.uint8)
    img = cv2.imdecode(buf, cv2.IMREAD_GRAYSCALE)
    # resize 到 128x128 与 Gradio app 默认行为一致
    img = cv2.resize(img, (128, 128))

    dims = {}

    # HOG
    h = hog(img, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(2, 2),
    ...
    # GLCM
    img_q = (img / 32).astype(np.uint8)  # 量化到 8 级
    glcm = graycomatrix(img_q, distances=[1], angles=[0], levels=8, symmetric=True, normed=True)
    props = [graycoprops(glcm, p) for p in ("contrast", "dissimilarity", "homogeneity",
                                             "energy", "correlation", "ASM")]
    glcm_vec = np.concatenate([p.ravel() for p in props])
    dims["GLCM"] = len(glcm_vec)
    assert len(glcm_vec) > 0, "GLCM 特征为空"
...
- [ ] **Step 2: Register Stage 2 in `main()`**
...
Run: `conda activate bjtu_ml && python scripts/e2e_test.py`
Expected: Stage 2 prints `✅ PASS  HOG:... LBP:... GLCM:... ED:...`.

- [ ] **Step 4: Commit**

```bash
git add scripts/e2e_test.py
git commit -m "feat: e2e test Stage 2 feature extraction verification"
```

---

### Task 4: Stage 3 — Traditional model inference

**Files:**
- Modify: `scripts/e2e_test.py`

- [ ] **Step 1: Add Stage 3 function**
...
def stage3_trad_models() -> str:
    ...
    # Edge density
    edges = cv2.Canny(img, 50, 150)
    ed = np.array([np.sum(edges > 0) / edges.size])

    feature_vec = np.concatenate([h, hist, glcm_vec, ed]).reshape(1, -1)
...
- [ ] **Step 2: Register Stage 3 in `main()`**
...
Run: `conda activate bjtu_ml && python scripts/e2e_test.py`
Expected: Stage 3 prints `✅ PASS  4/4 models OK`.

- [ ] **Step 4: Commit**

```bash
git add scripts/e2e_test.py
git commit -m "feat: e2e test Stage 3 traditional model inference"
```

---

### Task 5: Stage 4 — CNN inference

**Files:**
- Modify: `scripts/e2e_test.py`

- [ ] **Step 1: Add Stage 4 function**
...
- [ ] **Step 2: Register Stage 4 in `main()`**
...
Run: `conda activate bjtu_ml && python scripts/e2e_test.py`
Expected: Stage 4 prints `✅ PASS  probs=[..., ..., ...]`.

- [ ] **Step 4: Commit**

```bash
git add scripts/e2e_test.py
git commit -m "feat: e2e test Stage 4 CNN inference verification"
```

---

### Task 6: Stage 5 — Gradio module check

**Files:**
- Modify: `scripts/e2e_test.py`

- [ ] **Step 1: Add Stage 5 function**
...
- [ ] **Step 2: Register Stage 5 in `main()`**
...
Run: `conda activate bjtu_ml && python scripts/e2e_test.py`
Expected: Stage 5 prints `✅ PASS  interface created, run_pipeline callable`.

- [ ] **Step 4: Commit**

```bash
git add scripts/e2e_test.py
git commit -m "feat: e2e test Stage 5 Gradio module check"
```

---

### Task 7: Stage 6 — Gradio pipeline combination testing

**Files:**
- Modify: `scripts/e2e_test.py`

- [ ] **Step 1: Add Stage 6 function**

这是最核心的测试——直接调用 `run_pipeline` 模拟用户在 Gradio 界面上选择不同方法组合并运行。
...
def _build_pipeline_args(**overrides):
    ...
    d = defaults
    return (
        d["split_method"], d["split_ratio"], d["use_stratify"],
        d["preprocessing"], d["features"], d["max_samples"],
        d["model_name"],
        d["dt_max_depth"], d["dt_min_samples_split"],
        d["svm_C"], d["nb_var_smoothing"],
        d["rf_n_estimators"], d["rf_max_depth"], d["rf_min_samples_split"],
        d["lr_C"],
        d["xgb_n_estimators"], d["xgb_max_depth"], d["xgb_subsample"],
        d["lgbm_n_estimators"], d["lgbm_max_depth"], d["lgbm_num_leaves"],
        d["cnn_dropout"], d["cnn_batch_size"], d["cnn_epochs"],
        d["cnn_early_stopping"], d["cnn_input_size"], d["cnn_weight_decay"],
        d["unsup_n_clusters"], d["unsup_eps"], d["unsup_min_samples"],
        d["dt_criterion"],
        d["svm_kernel"], d["svm_gamma"],
        d["rf_criterion"],
        d["lr_penalty"], d["lr_solver"], d["lr_l1_ratio"],
        d["xgb_objective"], d["xgb_learning_rate"],
        d["lgbm_objective"], d["lgbm_learning_rate"],
        d["cnn_loss_fn"], d["cnn_focal_alpha"], d["cnn_focal_gamma"],
        d["cnn_label_smoothing_epsilon"], d["cnn_optimizer"], d["cnn_learning_rate"],
        d["kmeans_algorithm"], d["gmm_covariance_type"],
        d["agg_linkage"], d["spec_affinity"],
        d["optimization_strategy"], d["cv_folds_opt"], d["n_iter"],
        d["validation_method"], d["scoring_metric"],
        d["unsup_val_method"],
        d["random_seed"],
    )
...
- [ ] **Step 2: Register Stage 6 in `main()`**
...
Run: `conda activate bjtu_ml && python scripts/e2e_test.py`
Expected: All 7 stages pass, total time ~5-15 minutes. Report shows `7/7 PASS`.

- [ ] **Step 4: Commit**

```bash
git add scripts/e2e_test.py
git commit -m "feat: e2e test Stage 6 Gradio pipeline combination testing"
```
...
- [ ] **Step 1: Run the complete test end-to-end one more time**

Run: `conda activate bjtu_ml && python scripts/e2e_test.py`

Verify the full report:
```
====================================
  裂纹图像识别系统 — 端到端测试报告
====================================
  ✅ Stage 0: 环境检查         torch X.Y.Z, CUDA: True/False
  ✅ Stage 1: 数据路径验证     N images, 抽样读取 OK
  ✅ Stage 2: 特征提取验证     HOG:... LBP:... GLCM:... ED:...
  ✅ Stage 3: 传统模型推理     4/4 models OK
  ✅ Stage 4: CNN 推理         probs=[...]
  ✅ Stage 5: Gradio 模块检查  interface created, run_pipeline callable
  ✅ Stage 6: 方法组合运行     3/3 combos OK
────────────────────────────────────
  总计: 7/7 PASS  |  耗时: ~XXXs
====================================
```
...
```bash
ruff check scripts/e2e_test.py
ruff format scripts/e2e_test.py
```
...
```bash
git add scripts/e2e_test.py
git commit -m "refactor: format e2e_test.py with ruff"
```
